            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        @if($category->children->count() > 0)
                            <p class="text-xs text-red-500 mt-1">This category has sub-categories and cannot become a sub-category</p>
                        @endif
                    </div>
                ...
                <div>
                    ...
                    <textarea name="description" 
                              rows="4"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500">{{ old('description', $category->description) }}</textarea>
                </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="meta_description" 
                                      rows="2"
                                      maxlength="160"
                                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500">{{ old('meta_description', $category->meta_data['meta_description'] ?? '') }}</textarea>
                        </div>
                ...
                <div>
                    ...
                    @if($category->children->count() > 0)
                        <p class="text-xs text-yellow-600 mt-1">
                            ...
                            Changing status will also affect {{ $category->children->count() }} sub-categories
                        </p>
                    @endif
                </div>
                ...
                <div class="bg-blue-50 p-4 rounded-lg">
                    <h3 class="text-sm font-semibold text-gray-700 mb-2">Category Statistics</h3>
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                        ...
                        <div>
                            ...
                            <span class="font-semibold">{{ $category->children->count() }}</span>
                        </div>
                        <div>
                            <span class="text-gray-600">Created:</span>
                            <span class="font-semibold">{{ $category->created_at->format('M d, Y') }}</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) {
            preview.innerHTML = `
                ...
                <img src="${e.target.result}" class="w-32 h-24 object-cover border rounded">
            `;
        }
