    ...
    <div class="bg-white rounded-lg shadow-sm">
        <div class="p-6 border-b">
            <div class="flex items-center">
                ...
                <h1 class="text-2xl font-bold text-gray-800">Edit Brand: {{ $brand->name }}</h1>
            </div>
        </div>
            ...
            <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="mb-2">
                            ...
                            <img src="{{ Storage::url($brand->logo_url) }}" 
                                 alt="{{ $brand->name }}"
                                 class="w-20 h-20 object-contain border rounded bg-gray-50 p-2">
                        </div>
                        @endif
                    ...
                    <div>
                        ...
                        <select name="country" 
                            ...
                            @foreach($countries as $code => $name)
                                <option value="{{ $code }}" {{ old('country', $brand->country) == $code ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <textarea name="description" 
                              rows="4"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500">{{ old('description', $brand->description) }}</textarea>
                </div>
                ...
                <div class="bg-blue-50 p-4 rounded-lg">
                    <h3 class="text-sm font-semibold text-gray-700 mb-2">Brand Statistics</h3>
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <span class="text-gray-600">Total Products:</span>
                            <span class="font-semibold">{{ $brand->products()->count() }}</span>
                        </div>
                        <div>
                            <span class="text-gray-600">Created:</span>
                            <span class="font-semibold">{{ $brand->created_at->format('M d, Y') }}</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) {
            preview.innerHTML = `
                ...
                <img src="${e.target.result}" class="w-20 h-20 object-contain border rounded bg-gray-50 p-2">
            `;
        }
